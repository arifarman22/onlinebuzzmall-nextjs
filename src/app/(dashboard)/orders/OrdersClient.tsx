...
import { ShoppingCart, CheckCircle, Clock, TrendingUp, DollarSign, Percent, Lock } from 'lucide-react';
...
const VIP_CONFIG: Record<number, { label: string; color: string; bg: string; border: string; activeBg: string }> = {
  1: { label: 'VIP 1', color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/30', activeBg: 'bg-amber-500/20' },
  2: { label: 'VIP 2', color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/30', activeBg: 'bg-blue-500/20' },
  3: { label: 'VIP 3', color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/30', activeBg: 'bg-purple-500/20' },
};

// Balance thresholds per platform name
function getUnlockThreshold(name: string): number {
...
export default function OrdersClient({ user, platforms, assignments, stats }: Props) {
  ...
  // Always show all VIP levels 1, 2, 3
  const allVipLevels = [1, 2, 3];
  ...
  return (
    <div className="space-y-4">
      ...
      {/* VIP Level Buttons — always show All + VIP1 + VIP2 + VIP3 */}
        ...
        {allVipLevels.map((vip) => {
          ...
          return (
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl text-xs font-bold border transition-all ${
                isActive
                  ? `${cfg.activeBg} ${cfg.color} ${cfg.border}`
                  : 'bg-slate-900 text-slate-500 border-slate-800 hover:border-slate-700'
              }`}
            >
              {cfg.label}
      ...
      <div className="space-y-4">
        {filteredPlatforms.length === 0 ? (
          ...
          filteredPlatforms.map((platform) => {
            ...
            const completedPct = platformAssignments.length > 0
              ? platformAssignments.reduce((s, a) => s + a.percentage_completed, 0) / platformAssignments.length
              : 0;
            const cfg = VIP_CONFIG[platform.vip_level] || VIP_CONFIG[1];

            return (
              <div
                key={platform.id}
                className={`rounded-2xl border p-5 transition-all ${
                  isUnlocked ? 'bg-slate-900 border-slate-800' : 'bg-slate-900/50 border-slate-800/50 opacity-70'
                }`}
              >
                <div className="flex items-center gap-4 mb-4">
                  {getImageUrl(platform.image) ? (
                    ...
                    <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${cfg.bg}`}>
                      <ShoppingCart size={22} className={cfg.color} />
                    </div>
                  )}
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-slate-800 rounded-xl p-3 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[9px] uppercase tracking-wide text-slate-500 font-medium">Price Range</p>
                ...
                {!isUnlocked ? (
                  <div className="w-full flex items-center justify-center gap-2 py-3 bg-slate-800 text-slate-500 text-sm font-medium rounded-xl">
                    <Lock size={14} /> {getUnlockLabel(platform.name)}
                  </div>
                ) : hasAssignment ? (
                  <Link
