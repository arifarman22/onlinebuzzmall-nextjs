...
export default function AdminCreateAgentPage() {
  ...
  async function handleVerifyOTP(e: React.FormEvent) {
    ...
    const res = await fetch('/api/admin/users/verify-agent-otp', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    setTimeout(() => router.push('/admin/users?filter=agent'), 1500);
  }
        ...
        <CardContent>
          ...
          {step === 'otp' && (
            <form onSubmit={handleVerifyOTP} className="space-y-4">
              ...
              <p className="text-xs text-gray-400">Agent account will only be created after successful verification.</p>
              <div>
              ...
              <button type="submit" disabled={loading} className="w-full py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 disabled:opacity-50">
                {loading ? 'Verifying...' : 'Verify & Create Agent'}
              </button>
