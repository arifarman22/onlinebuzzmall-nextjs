...
import { pendingAgents } from '../create-agent/route';
...
export async function POST(req: NextRequest) {
  ...
  const pending = pendingAgents.get(token);
  if (!pending) {
  ...
  const { firstname, lastname, username, email, password, referral, plan_id } = pending.data;
