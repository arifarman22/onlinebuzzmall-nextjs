...
export default function AdminNotificationBell() {
  ...
  const handleClick = (n: AdminNotif) => {
    setOpen(false);
    router.push(`/admin/notifications/log/${n.id}`);
  };
        ...
        <>
          ...
          <div className="absolute right-0 top-12 z-50 w-80 bg-white rounded-xl border border-gray-200 shadow-xl overflow-hidden">
            ...
            <div className="max-h-80 overflow-y-auto">
              {notifications.length === 0 ? (
                ...
                <div
                  ...
                  className={`px-4 py-3 border-b border-gray-50 cursor-pointer hover:bg-gray-50 transition-colors ${n.is_read === 0 ? 'bg-indigo-50/50' : ''}`}
                >
                  <div className="flex items-start gap-2">
                    <span className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 ${n.is_read === 0 ? 'bg-indigo-500' : 'bg-gray-200'}`} />
            ...
            <Link
              ...
              className="flex items-center justify-center gap-1.5 px-4 py-3 text-xs font-medium text-indigo-600 hover:bg-indigo-50 border-t border-gray-100 transition-colors"
            >
