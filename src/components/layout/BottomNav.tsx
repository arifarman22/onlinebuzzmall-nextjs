...
export default function BottomNav() {
  ...
  return (
    <>
      ...
      {/* Desktop bottom nav — offset by sidebar width */}
      <div className="hidden lg:block" style={{ position: 'fixed', bottom: 0, left: '240px', right: 0, zIndex: 50, background: '#0f172a', borderTop: '1px solid #1e293b' }}>
        <div style={{ display: 'flex', height: '56px', width: '100%' }}>
          {navItems.map((item) => {
            const isActive = pathname === item.href || pathname.startsWith(item.href + '/');
            return (
